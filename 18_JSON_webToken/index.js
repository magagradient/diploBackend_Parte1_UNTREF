require("dotenv").config(); // para leer lo que estan en el .env
const express = require('express');
const app = express();
const jwt = require('jsonwebtoken');
const secretKey = process.env.SECRET_KEY // clave secreta que vamos a utilizar para firmar el token
...
app.post('/login', (req, res) => {
    ...
    if (email === userToValidate.email && password === userToValidate.password) {
        const token = jwt.sign({ id: userToValidate.id }, secretKey, { expiresIn: "1h" });
...
function verifyToken(req, res, next) {
    ...
    if (token) {
        ...
        res.status(401).json({ error: "No se envio el token" })
    }
};
